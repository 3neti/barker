...
it('can inject hyperverge headers result_endpoint', function () {
    ...
    expect(app(RetrieveResult::class)->hyperverge->resultEndpoint())->toBe(config('domain.hyperverge.api.url.result'));
});

it('can retrieve the result', function () {
    /*** arrange ***/
    Queue::fake();
    Event::fake();
    ...
    $url = $this->faker->url();
    $mobile = '+639171234567';
    $input = compact( 'url', 'mobile');
    $checkin = app(CreateCheckin::class)->run($agent, $input);

    $json = mockResultJsonResponse($checkin->uuid);
    $array = json_decode($json, true);

    $action = app(RetrieveResult::class);
    Http::fake([
        $action->hyperverge->resultEndpoint() => Http::response($json, 200)
    ]);
    ...
    Http::assertSent(function (Request $request) use ($checkin) {
        return
            ...
            && $request->url() == app(Hyperverge::class)->resultEndpoint()
...
//    dd($checkin->getKYC());
//    dd($checkin->getKYC()->application->modules[HypervergeModule::ID_VERIFICATION->value]->apiResponse->result->details->idType);
//    $kyc = \App\Data\Hyperverge\KYCData::from(mockResultJsonResponse('ABC123'));
//    dd($kyc->application->modules[HypervergeModule::ID_VERIFICATION->value]->apiResponse->result->details->fieldsExtracted);
});

function mockResultJsonResponse_working(string $transactionId): string
{
    return <<<EOT
{
    "status": "success",
